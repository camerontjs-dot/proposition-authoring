...
def main() -> None:
    parser = argparse.ArgumentParser(prog="run-gate-v1-rc3")
    ...
    task_raw = dict(packet.get("evidence_task", {}))
    if "known_gaps" in task_raw:
        task_raw["known_gaps"] = tuple(task_raw["known_gaps"])
    evidence_task = EvidenceTaskV1(**task_raw)
    ...
    _write(out_dir / "BUNDLE-VERIFICATION.json", verified)
